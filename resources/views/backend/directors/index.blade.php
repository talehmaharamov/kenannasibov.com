                ...
                <div class="row">
                    <div class="col-12">
                        <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                            ...
                            <a href="{{ route('backend.directors.create') }}"
                        ...
                        <table id="datatable-buttons"
                            ...
                            <tr>
                                ...
                                @foreach(active_langs() as $lan)
                                    ...
                                    <th>@lang('backend.position')({{ $lan->code }}):</th>
                                    <th>@lang('backend.description')({{ $lan->code }}):</th>
                                @endforeach
                            ...
                            @foreach($directors as $director)
                                <tr>
                                    <td>{{ $director->id }}</td>
                                    <td><img src="{{ asset($director->photo) }}" width="64" height="64"></td>

                                    @foreach(active_langs() as $lan)
                                        <td>{{ $director->translate($lan->code)->name }}</td>
                                        <td>{{ $director->translate($lan->code)->position }}</td>
                                        <td>{{ $director->translate($lan->code)->description }}</td>
                                    @endforeach
                                    <td class="text-center">
                                        <a href="{{ route('backend.directorStatus',['id'=>$director->id]) }}"
                                           class="btn btn-{{ $director->status == 1 ? 'success' : 'secondary' }}">{{ $director->status == 1 ? __('backend.active') : __('backend.deactive') }}</a>
                                    </td>
                                    <td class="text-center">
                                        <a class="btn btn-info"
                                           href={{ route('backend.directors.edit',['director'=>$director->id]) }}>
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a class="btn btn-danger"
                                           href="{{ route('backend.delDirector',['id'=>$director->id]) }}">
