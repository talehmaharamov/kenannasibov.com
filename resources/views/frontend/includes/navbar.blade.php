                    ...
                    <div class="header-row">
                        <div class="header-logo">
                            <a href="{{ route('frontend.index') }}" class="logo pe-0 pe-lg-3">
                                <img alt="Dırnıs Xeyriyyə Cəmiyyəti"
                                     src="{{  asset('frontend/images/logos/logo.png') }}" width="80" height=80"
                                     data-plugin-options="{'appearEffect': 'fadeIn'}" data-sticky-top="25">
                            </a>
                        </div>
                    </div>
                </div>
                                    ...
                                    <a class="nav-link" href="#" role="button" id="dropdownLanguage"
                                       data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                        <img
                                            ...
                                            class="flag flag-us"
                                            alt="{{ \App\Models\SiteLanguage::where('code',app()->getLocale())->first()->name }}"/> {{ Str::upper(\App\Models\SiteLanguage::where('code',app()->getLocale())->first()->code)  }}
                                            ...
                                            @foreach(active_langs() as $lang)
                                                <a class="dropdown-item"
                                                   href="{{ route('frontend.frontLanguage',$lang->code) }}"><img
                                                        src="{{ asset($lang->icon) }}" class="flag flag-us"
                                                        alt="{{ $lang->name }}"/> {{ Str::upper($lang->code) }}</a>
                                            @endforeach
                                        @endif
                                    </div>
                                </li>
                            ...
                            <ul class="header-social-icons social-icons social-icons-clean social-icons-icon-gray">
                                @if(settings('facebook') != '')
                                    <li class="social-icons-facebook">
                                        <a href="{{ settings('facebook') }}" target="_blank" title="Facebook"><i
                                                class="fab fa-facebook-f"></i></a>
                                    </li>
                                @endif
                                @if(settings('instagram') != '')
                                    <li class="social-icons-instagram">
                                        <a href="{{ settings('instagram') }}" target="_blank" title="Instagram"><i
                                                class="fab fa-instagram"></i></a>
                                    </li>
                                @endif
                                @if(settings('telegram') != '')
                                    <li class="social-icons-telegram">
                                        <a href="{{settings('telegram')}}" target="_blank" title="Telegram"><i
                                                class="fab fa-telegram"></i></a>
                                    </li>
                                @endif
                                @if(settings('youtube') != '')
                                    <li class="social-icons-youtube">
                                        <a href="{{settings('youtube')}}" target="_blank" title="YouTube"><i
                            ...
                            <div class="header-nav-main header-nav-main-effect-1 header-nav-main-sub-effect-1">
                                <nav class="collapse">
                                    <ul class="nav nav-pills" id="mainNav">
                                        ...
                                        <li class="dropdown">
                                            <a class="dropdown-item dropdown-toggle {{ ( Route::currentRouteName()  == 'frontend.allPosts' or Route::currentRouteName()  == 'frontend.post') ? 'active' : '' }}"
                                               href="{{ route('frontend.allPosts') }}">
                                                @lang('backend.news')
                                            </a>
                                        </li>
                                        <li class="dropdown">
                                            <a class="dropdown-item dropdown-toggle {{ ( Route::currentRouteName()  == 'frontend.selectedCategory' or Route::currentRouteName()  == 'frontend.selectedPost') ? 'active' : '' }}"
                                               href="#">
                                            ...
                                            <ul class="dropdown-menu">
                                                @foreach($categories as $category)
                                                    @if($category->slug != 'news' and count(check_category($category->id)))
                                                        <li>
                                                            <a class="dropdown-item"
                                                               href="{{ route('frontend.selectedCategory',$category->slug) }}">{{ $category->translate($locale)->name }}</a>
                                                        </li>
                                                    @endif
                                                @endforeach
                                            </ul>
